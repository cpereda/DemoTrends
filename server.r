...
server <- function(input, output, session) {
  ...
  output$map <- renderLeaflet({
    ...
    if ("Uninsured" %in% input$other) {
      map <- map %>%
        addCircles(
          lng = ~Longitude,
          lat = ~Latitude,
          data = joineddf3_sf %>% filter(fips != 15005 & 08014), # Filter out the row with FIPS 15005
    ...
    #Addmarker for specific county with population size
    map <- map %>%
      addMarkers(
        lng = ~Longitude,
        lat = ~Latitude,
  ...
  #ggplot of states and counties with selected health indicator
  output$state_map <- renderPlot({
    ...
    joineddf2 %>%
      ...
      geom_polygon() +
      scale_fill_gradient(low = "white", high = "blue", name = paste(input$health, "Log"),
                          trans="log") +
